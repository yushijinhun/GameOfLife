	...
	private static LifeGameEngine createEngine() {
		if (filePath != null) {
			...
			try {
				...
				ExceptionUtil.showExceptionDialog(e, Thread.currentThread(),
						"When loading the game, an IOException occurred.\n");
				throw new IOError(e);
			} finally {
				if (in != null) {
					try {
		...
		return new LifeGameEngine(new LifeGameEngineConfiguration(width,
				height, threads));
	}
