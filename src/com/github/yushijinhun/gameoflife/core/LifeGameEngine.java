...
public class LifeGameEngine {
	...
	public static LifeGameEngine readFromNBT(NbtTagCompound comp, int threads) {
		LifeGameEngine game = new LifeGameEngine(
				new LifeGameEngineConfiguration(comp.getInt("width"),
						comp.getInt("height"), threads));
	...
	public LifeGameEngine(LifeGameEngineConfiguration config) {
		...
		if (!singleThread) {
			...
			for (int i = 0; i < threads; i++) {
				if (i == (threads - 1)) {
					computingUnits[i] = new ComputingUnit(i * rowsEveryUnit,
							width);
				} else {
					computingUnits[i] = new ComputingUnit(i * rowsEveryUnit,
							rowsEveryUnit * (i + 1));
