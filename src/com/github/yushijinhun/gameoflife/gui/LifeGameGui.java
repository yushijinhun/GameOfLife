...
public class LifeGameGui extends Canvas {
		...
		public void run() {
			while (buffer == null) {
				if (getGraphicsConfiguration() != null) {
					...
					buffer = getGraphicsConfiguration().createCompatibleImage(
							(int) d.getWidth(), (int) d.getHeight());
					cellsBuffer = getGraphicsConfiguration()
							.createCompatibleImage(engine.width, engine.height);
					...
					engine.setCellHandler(new LifeGameCellHandler() {
...

						{
							...
							deadData = colorModel.getDataElements(deadColor,
									null);
							livingData = colorModel.getDataElements(
									livingColor, null);
							sampleModel = cellsBuffer.getRaster()
									.getSampleModel();
							dataBuffer = cellsBuffer.getRaster()
									.getDataBuffer();
						}

						@Override
						public void onChanged(LifeGameEngine engine, int x,
								int y, boolean to) {
							sampleModel.setDataElements(x, y, to ? livingData
									: deadData, dataBuffer);
						}
					});
	...
	private class SavingThread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				ExceptionUtil.showExceptionDialog(e1, Thread.currentThread(),
						"When saving the game, an IOException occurred.\n");
			} finally {
				if (out != null) {
					try {
						out.close();
			...
			JOptionPane.showMessageDialog(LifeGameGui.this,
					"The game saved to " + file.getPath(), "Game of Life",
	...
	public LifeGameGui(double blockSize, LifeGameEngine theEngine) {
		...
		addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				if (isPressing) {
					...
					if ((x >= engine.width) || (y >= engine.height) || (x < 0)
							|| (y < 0)) {
						return;
					}
		...
		setSize((int) (Math.rint(engine.width * scale)),
				(int) (Math.rint(engine.height * scale)));
		renderThread.start();
	}
	...
	private void render(Graphics2D g2d) {
		g2d.drawImage(cellsBuffer, xOffset, yOffset,
				(int) (Math.rint(cellsBuffer.getWidth() * scale)),
				(int) (Math.rint(cellsBuffer.getHeight()) * scale), null);

		if (showInfo) {
			g2d.setFont(font);
			renderString(g2d, "computing threads: " + engine.getThreads(), 0,
					60);
			renderString(g2d, "memory: "
					+ ((Runtime.getRuntime().totalMemory() - Runtime
							.getRuntime().freeMemory()) / 1024 / 1024)
							+ "m used", 0, 50);
			renderString(g2d, "cell size: " + scale, 0, 40);
			renderString(g2d, "last ticking time: "
					+ (isComputing ? "computing..." : lastTickingTime), 0, 30);
			renderString(g2d, "ticks: " + engine.getTicks(), 0, 20);
			renderString(g2d, "fps: " + fps, 0, 10);
			renderString(g2d, "(" + mouseRelativeX + ", " + mouseRelativeY
					+ ")", mouseX + 10, mouseY + 10);
		}
